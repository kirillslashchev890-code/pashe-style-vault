...
const FAQ: Record<string, string> = {
  ...
  "контакт": "📧 Свяжитесь с нами: support@uventus.ru. Вы также можете написать в этот чат, и оператор ответит вам.",
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      setMessages([{
        id: "welcome",
        role: "bot",
        content: "👋 Здравствуйте! Я бот-помощник ЮВЕНТУС. Напишите «помощь» для списка доступных команд, или задайте вопрос напрямую.",
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-[360px] h-[500px] bg-card border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-border bg-primary text-primary-foreground rounded-t-2xl">
            <span className="font-semibold">Поддержка ЮВЕНТУС</span>
