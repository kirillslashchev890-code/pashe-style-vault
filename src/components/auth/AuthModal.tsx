...
const AuthModal = ({ isOpen, onClose, onSuccess }: AuthModalProps) => {
  ...
  const resetForm = () => {
    ...
    setGeneralError(""); setAgreePolicy(false);
  };
          ...
          {mode === "forgot" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <Input id="forgot-email" type="email" placeholder="email@example.com" className={`pl-10 h-12 ${emailError ? "border-destructive" : ""}`}
                ...
                <Button type="submit" className="w-full btn-gold h-12" disabled={isLoading}>
                  {isLoading ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin" />Отправка...</span> : "Отправить ссылку"}
                </Button>
              </form>
              <button onClick={() => switchMode("login")} className="flex items-center gap-1 text-primary text-sm hover:underline mx-auto">
            ...
            <>
              ...
              <div className="mt-4 text-center space-y-2">
                {mode === "login" && (
                  <button onClick={() => switchMode("forgot")} className="text-primary text-sm hover:underline">
                    Забыли пароль?
                  </button>
                )}
